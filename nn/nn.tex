...
As computer engineers we could look at the neuron as a special type of logic gate: the dendrites represent the different inputs, the axon represents the output and the combination of the synapses and the bias (the inverse of the threshold) specify which type of gate it is. Figure \ref{fig:AN} shows a representation of an artificial neuron. It has three inputs. In general it can have more or less inputs $x_1, \dots, x_n$.
...
To compute the output we assign to each input a weight, $w_1, \dots, w_n$. The weights represent the importance of the respective inputs to the output.

Additionally, we assign a \textit{bias} $b$ to the neuron. Just like the weights, the bias is a real number which is a parameter of the neuron. You can think of the bias as a measure of how easy it is to get the perceptron to output a 1. It is the inverse of the concept threshold: $b = -threshold$. 

Or to put it in more biological terms, the bias is a measure of how easy it is to get the perceptron to fire. For a perceptron with a really big bias, it's extremely easy for the perceptron to output a 1. But if the bias is very negative, then it's difficult for the perceptron to output a 1.

The neuron's output, 0 or 1, is determined by whether the weighted sum $\sum^n_{i=1} w_i x_i + b$ is less than or greater than 0. To put it in more precise algebraic terms:
...
We can now make an artificial neuron that can act as a logic gate (see figure \ref{fig:neuron_and_gate}, \ref{fig:neuron_or_gate} and \ref{fig:neuron_invert_gate}). By assigning the weights and the bias we decide on the behaviour.

\begin{figure}[h!]
\centering
\begin{tikzpicture}[scale=1, transform shape]
\node (a)[circle, draw] at (3, 0) {$b = -1.0$};
\node (x1)[] at (0, 1) {$x_1$};
\node (x2)[] at (0, -1) {$x_2$};
...
\caption{Appropriate weights and bias to act as an AND-gate.}
\label{fig:neuron_and_gate}
\end{figure}
\begin{figure}[h!]
\centering
\begin{tikzpicture}[scale=1, transform shape]
\node (a)[circle, draw] at (3, 0) {$b = -0.5$};
\node (x1)[] at (0, 1) {$x_1$};
\node (x2)[] at (0, -1) {$x_2$};
\node (output)[] at (5, 0) {output};
\draw[->] (x1) -- (a) node[pos=0.3,sloped,above] {\small $w_1 = 0.5$};
...
\caption{Appropriate weights and bias to act as an OR-gate.}
...
\caption{Appropriate weights and bias to act as an INVERT-gate.}
...
\subsection{Using vectors}%\todo{andere titel?}
In section \ref{sec:ArtificialNeuron} we described the working of a perceptron in equation \ref{eq:AF_perceptron} using inputs $x_1 \dots x_n$, corresponding weights $w_1 \dots w_n$ and bias $b$. The condition $\sum_{i=1}^n w_i x_i + b > 0$ is cumbersome, and we can make a notational change to simplify it. The first change is to write it as a inner product, $\inprod{w}{x} = \sum_{i=1}^n w_i x_i$, where $\vec{w}$ and $\vec{x}$ are \textit{vectors} whose components are the weights and inputs, respectively. 

The second change is to simplify the equation \ref{eq:AF_perceptron} and our calculations a bit by making the bias a weight of the special input $x_0$ which will always be 1. This will give us the perceptron activation function, $g$,:

\begin{equation}
\mathit{output} = g = \left\{\begin{matrix}
 0 & \text{if } \inprod{w}{x} < 0\\
 1 & \text{if } \inprod{w}{x} \geq 0
\end{matrix}\right.
...
This change simplifies the calculations needed in neural networks, as now only vector multiplications are required. Later, in Section \ref{ch:opencl}, this means that we can more easily offload the neural network calculations to the GPU for training.
...
\begin{equation}
\mathit{output} = g = \sigma\big(\inprod{w}{x}\big)
...
\begin{equation}
MSE = \frac{1}{n} \sum_{i=0}^n \big \Vert\,\vec{y_i} - \vec{a(x_i)} \big \Vert^2
\label{eq:MSE}
\end{equation}

The formula for this cost function $\vec{C(w)}$ calculates the error (cost $C$) given a vector of weights ($\vec{w}$):\footnote{The maths for this cost function might seem complicated, but with a closer look we see it is literally the mean squared error. $\big\Vert\,\vec{y_i} - \vec{a(x_i)} \big\Vert$ is the difference (or distance) between the output that is given and the output that we want when given input $\vec{x_i}$. In other words $\big\Vert\,\vec{y_i} - \vec{a(x_i)} \big\Vert $ tells us how wrong the neural network is (i.e. how big the error is) when given input $\vec{\vec x}_i$. As this value can be negative or positive and we are only interested in how wrong the neural network is, we square this value to make all values positive. This is the squared error part of the mean squared error. To get to MSE, we need to calculated the mean. This is done by summing ($\sum$) the squared error over all training inputs and dividing it by the number of training inputs, $i=1,\ldots,n$, ($\frac{1}{2n}$). Actually we also divide by two (hence the 2 in $\frac{1}{2n}$). This has no effect on the functionality when minimising the function, but it makes the function derivative easier. As we use the derivative of this function this  saves us some clutter. In short the MSE is nothing more than the average squared distance between the output of the neural network and the desired output.}
...
\begin{equation}
\vec{C(w)} = \frac{1}{2} MSE = \frac{1}{2n} \sum_{i=0}^n \big \Vert\,\vec{y_i} - \vec{a(x_i)} \big \Vert^2
\label{eq:MSE}
\end{equation}
where $n$ is the number of training inputs, $x$, $\vec{y_i}$ is the desired outcome of the network when $\vec{x_i}$ is the input and $a(x_i)$ is the output of the neural network when $\vec{x_i}$ is the input. The notation $\big\Vert v \big\Vert$ denotes the usual length function for a vector $\vec{v}$. \footnote{If we have a vector $\vec{v}$ with values $v_1$, $v_2$ and $v_3$, then $\big \Vert v \big \Vert$ is equals to $\sqrt{\inprod{v}{v}} = \sqrt{{v_1}^2+{v_2}^2+{v_3}^2}$.}

Inspecting the form of the quadratic cost function, we see that $\vec{C(w)}$ is non-negative, since every term in the sum is non-negative. Furthermore, the cost $\vec{C(w)}$ becomes small, i.e., $\vec{C(w)}\approx 0$, precisely when $\vec{y_i}$ is approximately equal to the output, $a$, for all training inputs, $\vec{x_i}$. So our training algorithm has done a good job if it can find weights and biases so that $\vec{C(w)}\approx0$. By contrast, it's not doing so well when $\vec{C(w)}$ is large - that would mean that $\vec{y_i}$ is not close to the output $\vec{a(x_i)}$ for a large number of inputs. So the aim of our training algorithm will be to minimise the cost $\vec{C(w)}$ as a function of the weights and biases. In other words, we want to find a set of weights and biases which make the cost as small as possible\footnote{For some readers the questions might rise why we try to minimise the quadratic cost function, as we just as well could maximise the number of correct classified training samples. The problem with the number of correct classified training samples is that it is not a smooth function. Making a small change in the weights of the units will most of the time result in no change in the number of correct classified training samples. This makes it hard to find which change we should make to the weights of the network. The quadratic cost function is smooth and, therefore, does not have this problem.}.

\subsection{Gradient descent}\label{sec:gradient}
In section \ref{sec:cost_function}, we saw that the goal of a learning algorithm is to minimise the cost function $\vec{C(w)}$. As $\vec{w}$ consists out of a lot of weights it takes too much time to calculated for each possible $\vec{w}$ the value of $\vec{C(w)}$ to see where $\vec{C(w)}$ has the lowest value. A solution for this problem is to use gradient descent. Gradient descent is an optimisation algorithm that approaches a (local) minimum of a function by taking steps in the direction of the negative of the gradient of the function at the current point.
...
Back to our problem. We cannot calculate the minimum of the function, but we can start at a random position (random weights) and then use gradient descent to move down to a minimum. More formal, the algorithm starts with a random $\vec{w}$ and calculates which small change, $\Delta \vec{w}$, reduces $\vec{C(w)}$. In other words, we want to find a $\Delta \vec{w}$, that has a negative $\Delta \vec{C(w)}$. When $\Delta \vec{w}$ is applied to the weights $\vec{C(w)}$ comes closer to a minimum. When we keep repeating this step until we cannot find a $\Delta \vec{w}$, such that $\Delta \vec{C(w)}$ is negative, than we are at a (local) minimum and the network has learned its weights.
...
\begin{equation}
C = \frac{1}{2} \big\Vert\,\vec{y} - \vec{a(x)} \big\Vert^2
\end{equation}
...
& = w_i - \eta \frac{\partial(\frac{1}{2} \big\Vert\,\vec{y} - \vec{a(x)} \big\Vert^2)}{\partial w_i}
\end{split}
\end{equation}
...
Determine weights and a bias for a perceptron that would act as a NOR-gate with three inputs.
