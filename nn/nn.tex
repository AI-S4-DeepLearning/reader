...
Additionally, we assign a \textit{bias} $b$ to the neuron\footnote{Bias $b$ is the inverse of the aforementioned concept threshold $t$, so $b = -t$.}. Just like the weights, the bias is a real number which is a parameter of the neuron. You can think of the bias as a measure of how easy it is to get the perceptron to output a 1.  
...
\begin{equation}
w_{j,k} = w_{j,k} - \Delta w_{j,k} = w_{j,k} + \eta a_{j} \dot{g}(in_k)(y_k - a_k)
\end{equation}
where $a_j$ is the activation of input node $j$, $\dot{g}$ is the derivative of the activation function\footnote{For the sigmoid neuron the derivative is given by $\dot{g} = g(1-g)$}, $in_k$ is the summed input of output node $k$, $y_k$ is the desired activation of output node $k$ and $a_k$ is the actual activation of output node $k$\footnote{Note the change of sign in the equation: the (partial) derivative $\eta \frac{\partial(\frac{1}{2} \left | \vec{y} - \vec{a}(\vec{x}) \right |^2)}{\partial w_i}$ is negative (as we are looking for the down slope), which means that the sign changes from a minus to a plus.}. This updating rule is called the \textit{delta rule}.
...
\delta_j = \dot{g}(in_j) \sum_p w_{j,p}\, \delta_p
\end{equation}
