...
\begin{enumerate}[label=3.\arabic*, itemsep=8pt]
	...
	\item XOR-poort: zie figuur \ref{fig:xor_gate}.
		...
		\begin{tikzpicture}[scale=1, transform shape]
			...
			\node (a)[circle, draw, minimum width=40pt, label=OR] at (4, 2) {$b = -1$};
			\node (b)[circle, draw, minimum width=40pt, label=NAND] at (4, -2) {$b = 1$};
			\node (HL)[circle] at (4, -3) {Hidden layer};

			\node (c)[circle, draw, minimum width=40pt, label=AND] at (8, 0) {$b = -2$};
	...
	\item Half adder: zie figuur \ref{fig:adder}.
		...
		\begin{tikzpicture}[scale=1, transform shape]
			...
			\node (f)[circle, draw, minimum width=40pt, label=AND] at (5, 4) {$b = -2$};
			\node (g)[circle, draw, minimum width=40pt, label=OR] at (5, 0) {$b = -1$};
			\node (h)[circle, draw, minimum width=40pt, label=NAND] at (5, -4) {$b = 1$};
			\node (HL)[circle] at (5, -5) {Hidden layer};

			\node (c)[circle, draw, minimum width=40pt, label=BUFF] at (10, 2) {$b = -1$};
			\node (CL)[circle] at (11.5, 2) {Carry};
			\node (s)[circle, draw, minimum width=40pt, label=AND] at (10, -2) {$b = -2$};
		...
		\end{tikzpicture}
		\caption{Oplossing half-adder. De sum is een XOR en de carry is een AND. De bovenste outputnode is de carry
		en de onderste is de sum. De bovenste hidden node is de AND en de carry zelf doet niets (dit is ook bekend
		als de BUFFER poort). De onderste twee hidden nodes en de sumnode zijn samen een XOR (zie figuur \ref{fig:xor_gate}). 
		De grijze verbindingen hebben $w=0$.}
		\label{fig:adder}
		\end{figure}
